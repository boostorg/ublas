...
int main()
{
  ...
  auto B      = dynamic_tensor<ctype, layout::last_order>(
    dynamic_extents<>{5, 4, 3, 2}, ctype{});
  ...
  auto C = dynamic_tensor<ctype, layout::last_order>(B.extents());
  ...
  // computes the complex conjugate of elements of B
  // using iterators.
  auto D = dynamic_tensor<ctype, layout::last_order>(B.extents());
  std::transform(
    B.begin(), B.end(), D.begin(), [](auto const& b) { return std::conj(b); });
  std::cout << "% --------------------------- " << std::endl;
  std::cout << "% --------------------------- " << std::endl << std::endl;
