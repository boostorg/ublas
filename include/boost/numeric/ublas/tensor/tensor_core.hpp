...
public:
  ...
  using const_reverse_iterator =
    typename storage_traits_type::const_reverse_iterator;
  ...
  constexpr tensor_core(extents_type e,
                        [[maybe_unused]] storage_resizable_container_tag t)
  ...
  constexpr tensor_core(extents_type e,
                        [[maybe_unused]] storage_static_container_tag t)
    : tensor_expression_type<self_type>()
    , extents_(std::move(e))
    , strides_(extents_)
  {
    if (data_.size() < product(extents_)) {
      throw std::length_error(
        "boost::numeric::ublas::tensor_core(extents_type const&, "
        "storage_static_container_tag): "
        "size of requested storage exceeds the current container size");
    }
  }

  /** @brief Constructs a tensor_core with an initializer list for
   * dynamic_extents
   *
   ...
   *
   * @param l initializer list for setting the dimension extents of the
   * tensor_core
   */
  template <typename U = extents_type,
            typename   = std::enable_if_t<is_dynamic_v<U>>>
  explicit inline tensor_core(std::initializer_list<size_type> l)
  ...
  template <typename U = extents_type,
            typename   = std::enable_if_t<is_dynamic_v<U>>>
  explicit inline tensor_core(extents_type const& s)
  ...
  template <typename U = extents_type,
            typename   = std::enable_if_t<is_dynamic_v<U>>>
  explicit inline tensor_core(extents_type const& s, value_type const& i)
    : tensor_core(s, resizable_tag{})
  {
  ...
  explicit inline tensor_core(value_type const& i)
    : tensor_core()
  {
    std::fill_n(begin(), size(), i);
  }

  /** @brief Constructs a tensor_core with a \c shape and initiates it with
  ...
  template <typename U = extents_type,
            typename   = std::enable_if_t<is_dynamic_v<U>>>
  inline tensor_core(extents_type const& s, const array_type& a)
    : tensor_core(s, resizable_tag{})
  {
    if (size() != a.size()) {
      throw std::runtime_error(
        "boost::numeric::ublas::tensor_core(extents_type,array_type): "
  ...
  template <typename U = extents_type,
            typename   = std::enable_if_t<is_static_v<U>>>
  inline tensor_core(const array_type& a)
    : tensor_core()
  {
    if (size() != a.size()) {
      throw std::runtime_error(
  ...
  /** @brief Constructs a tensor_core with another tensor_core with a different
   * layout
   *
    ...
    : tensor_core(detail::retrieve_extents(expr), resizable_tag{})
  {
    static_assert(is_valid_tensor_v<other_tensor>,
                  "boost::numeric::ublas::tensor_core(tensor_expression<other_"
                  "tensor, derived_type> const&) : "
  ...
  {
    if constexpr (is_dynamic_v<extents_type>) {
      auto temp = tensor_core(
        extents_type{v.size(), typename extents_type::value_type{1}});
      swap(*this, temp);
    }
   ...
   */
  inline tensor_core(const tensor_core& v)
    : tensor_expression_type<self_type>()
    , extents_(v.extents_)
    , strides_(v.strides_)
    , data_(v.data_)
   ...
   */
  inline tensor_core(tensor_core&& v) noexcept
   ...
   *  @param v tensor_core to be moved.
   */
  inline tensor_core& operator=(tensor_core&& v) noexcept
  {
    swap(*this, v);
    return *this;
  }
  ...
  [[nodiscard]] inline constexpr size_type size(size_type r) const
  {
    return this->extents_.at(r);
  }

  /** @brief Returns the number of dimensions/modes of the tensor_core */
  [[nodiscard]] inline constexpr size_type rank() const noexcept
  {
    return this->extents_.size();
  }

  /** @brief Returns the number of dimensions/modes of the tensor_core */
  [[nodiscard]] inline constexpr size_type order() const noexcept
  {
    return this->extents_.size();
  }

  /** @brief Returns the strides of the tensor_core */
  [[nodiscard]] inline constexpr strides_type const& strides() const noexcept
  {
    return this->strides_;
  }

  /** @brief Returns the extents of the tensor_core */
  [[nodiscard]] inline constexpr extents_type const& extents() const noexcept
  {
    return this->extents_;
  }

  /** @brief Returns the strides of the tensor_core */
  [[nodiscard]] inline constexpr strides_type& strides() noexcept
  {
    return this->strides_;
  }

  /** @brief Returns the extents of the tensor_core */
  [[nodiscard]] inline constexpr extents_type& extents() noexcept
  {
    return this->extents_;
  }
  ...
  [[nodiscard]] inline constexpr array_type const& base() const noexcept
  {
    return data_;
  }

  /** @brief Returns a reference to the underlying container. */
  [[nodiscard]] inline constexpr array_type& base() noexcept { return data_; }
  ...
  [[nodiscard]] inline constexpr const_reference at(
    typename strides_type::value_type i, Indices... is) const
  {
    if constexpr (sizeof...(is) == 0ul) {
      ...
      auto const idx           = detail::access(
        this->strides_, i, static_cast<strides_value_type>(is)...);
   ...
   */
  template <class... Indices>
  [[nodiscard]] inline constexpr reference at(
    typename strides_type::value_type i, Indices... is)
  {
    if constexpr (sizeof...(is) == 0ul) {
      return this->data_.at(i);
    }
  ...
  [[nodiscard]] inline constexpr decltype(auto) operator()(
    index::index_type<I> p, index_types... ps) const
  {
    constexpr auto N = sizeof...(ps) + 1;
    if (N != this->rank())
      throw std::runtime_error(
        "Error in "
        "boost::numeric::ublas::operator(index::index_type<I>,index_types&&): "
   ...
   */
  inline void reshape(extents_type const& e, value_type v = value_type{})
  {
    static_assert(is_dynamic_v<extents_type>,
                  "Error in boost::numeric::ublas::basic_tensor: static "
                  "extents cannot be reshaped");
    this->extents_ = e;
  ...
  [[nodiscard]] inline constexpr iterator end() noexcept { return data_.end(); }

  /// \brief Return a const reverse iterator before the first element of the
  /// reversed tensor_core (i.e. end() of normal tensor_core)
  ...
  /// \brief Return a const reverse iterator on the end of the reverse
  /// tensor_core (i.e. first element of the normal tensor_core)
