...
template <class int_type> class basic_extents {
  static_assert(
    std::numeric_limits<typename std::vector<int_type>::value_type>::is_integer,
    "Static error in basic_layout: type must be of type integer.");
  static_assert(
    !std::numeric_limits<typename std::vector<int_type>::value_type>::is_signed,
...
public:
    ...
    : _base(std::move(b))
  {
    if (!valid(*this)) {
      throw std::length_error(
        "Error in basic_extents::basic_extents() : shape tuple is not a valid "
        "permutation: has zero elements.");
    }
  }
  ...
  basic_extents(OtherExtents const& e)
    : _base(e.size())
  {
    static_assert(is_extents_v<OtherExtents>,
                  "boost::numeric::ublas::basic_extents(OtherExtents const&) : "
  ...
  basic_extents& operator=(basic_extents const& other) noexcept(
    std::is_nothrow_swappable_v<base_type>)
  {
  ...
  [[nodiscard]] inline constexpr reference operator[](size_type p)
  {
    return this->_base[p];
  }

  [[nodiscard]] inline constexpr reference at(size_type p)
  {
    return this->_base.at(p);
  }
