          ...
          SizeType const* const wa)
{
  static_assert(std::is_pointer<PointerOut>::value &
                  std::is_pointer<PointerIn>::value,
                "Static error in boost::numeric::ublas::copy: Argument types "
  ...
  if (c == nullptr || a == nullptr)
    throw std::runtime_error(
      "Error in boost::numeric::ublas::copy: Pointers shall not be null "
      "pointers.");
  ...
  lambda = [&lambda, n, wc, wa](SizeType r, PointerOut c, PointerIn a) {
    if (r > 0)
      ...
      for (auto d = 0u; d < n[0]; c += wc[0], a += wa[0], ++d) *c = *a;
  };
...
/** @brief Copies a tensor to another tensor with different layouts applying a
 * unary operation
 *
               ...
               UnaryOp op)
{
  static_assert(std::is_pointer<PointerOut>::value &
                  std::is_pointer<PointerIn>::value,
                "Static error in boost::numeric::ublas::transform: Argument "
                "types for pointers are not pointer types.");
  if (p == 0)
    return;

  if (c == nullptr || a == nullptr)
    throw std::runtime_error(
      "Error in boost::numeric::ublas::transform: Pointers shall not be null "
      "pointers.");
  ...
  lambda = [&lambda, n, wc, wa, op](SizeType r, PointerOut c, PointerIn a) {
    if (r > 0)
      ...
      for (auto d = 0u; d < n[0]; c += wc[0], a += wa[0], ++d) *c = op(*a);
  };
...
/** @brief Performs a reduce operation with all elements of the tensor and an
 * initial value
 *
...
ValueType accumulate(SizeType const p,
                     SizeType const* const n,
                     PointerIn a,
                     SizeType const* const w,
                     ValueType k)
{
  ...
  lambda = [&lambda, n, w](SizeType r, PointerIn a, ValueType k) {
    if (r > 0u)
      for (auto d = 0u; d < n[r]; a += w[r], ++d) k = lambda(r - 1, a, k);
    else
      for (auto d = 0u; d < n[0]; a += w[0], ++d) k += *a;
    return k;
  };
                     ...
                     BinaryOp op)
{
  ...
  lambda = [&lambda, n, w, op](SizeType r, PointerIn a, ValueType k) {
    if (r > 0u)
      ...
      for (auto d = 0u; d < n[0]; a += w[0], ++d) k = op(k, *a);
    return k;
  };
           ...
           SizeType const* const wa)
{
  static_assert(std::is_pointer<PointerOut>::value &
                  std::is_pointer<PointerIn>::value,
                "Static error in boost::numeric::ublas::trans: Argument types "
                "for pointers are not pointer types.");
  ...
  if (c == nullptr || a == nullptr)
    throw std::runtime_error(
      "Error in boost::numeric::ublas::trans: Pointers shall not be null "
      "pointers.");
  ...
  lambda = [&lambda, na, wc, wa, pi](SizeType r, PointerOut c, PointerIn a) {
    if (r > 0)
      ...
      for (auto d = 0u; d < na[0]; c += wc[pi[0] - 1], a += wa[0], ++d) *c = *a;
  };
           ...
           SizeType const* const wa)
{
  ...
  std::function<void(
    SizeType r, std::complex<ValueType> * c, std::complex<ValueType> * a)>
    lambda;

  lambda = [&lambda, na, wc, wa, pi](SizeType r,
                                     std::complex<ValueType>* c,
                                     std::complex<ValueType>* a) {
    if (r > 0)
      for (auto d = 0u; d < na[r]; c += wc[pi[r] - 1], a += wa[r], ++d)
