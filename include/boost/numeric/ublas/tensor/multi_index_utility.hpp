...
template <class itype_left> struct has_index_impl<itype_left, std::tuple<>> {
...
struct has_index_impl<itype_left, std::tuple<itype_right, index_types...>> {
  ...
  static constexpr bool value =
    has_index_impl<itype_left, itype_right>::value || next_type::value;
};
}   // namespace detail
 ...
 *
 * @note a multi-index represents as tuple of single indexes of type
 * boost::numeric::ublas::index::index_type
 *
 * @code auto has_index_value = has_index<index_type<1>,
 * std::tuple<index_type<2>,index_type<1>> >::value; @endcode
 *
...
template <class index_type, class tuple_type> struct has_index {
  static constexpr bool value =
    detail::has_index_impl<std::decay_t<index_type>,
                           std::decay_t<tuple_type>>::value;
};
...
template <> struct valid_multi_index_impl<std::tuple<>> {
  static constexpr bool value = true;
};

template <class itype> struct valid_multi_index_impl<std::tuple<itype>> {
  static constexpr bool value = true;
};
...
struct valid_multi_index_impl<std::tuple<itype, index_types...>> {
  ...
  static constexpr bool has_index_value =
    has_index_type::value && !is_index_zero;
  static constexpr bool value =
 ...
 * boost::numeric::ublas::index::index_type
 *
 * @code auto valid = valid_multi_index< std::tuple<index_type<2>,index_type<1>>
 * >::value;
 * @endcode
 *
 * @tparam tuple_type type of std::tuple representing a multi-index
...
template <class tupe_type> struct valid_multi_index {
  static constexpr bool value =
    detail::valid_multi_index_impl<std::decay_t<tupe_type>>::value;
};
...
template <class... index_types> struct number_equal_indexes_impl;
...
struct number_equal_indexes_impl<std::tuple<itype, itypes_left...>,
                                 std::tuple<itypes_right...>> {
  ...
  using next_type =
    number_equal_indexes_impl<std::tuple<itypes_left...>, tuple_right>;
...
/** @brief number_equal_indexes contains the number of equal indexes of two
 * multi-indexes
 *
 * @note a multi-index represents as tuple of single indexes of type
 * boost::numeric::ublas::index::index_type
 ...
 */
template <class tuple_left, class tuple_right> struct number_equal_indexes {
  static constexpr unsigned value =
    detail::number_equal_indexes_impl<std::decay_t<tuple_left>,
...
struct index_position_impl {
  static constexpr auto is_same =
    std::is_same<std::decay_t<itype>,
                 std::decay_t<std::tuple_element_t<r, ttype>>>::value;
  static constexpr auto value =
    is_same ? r : index_position_impl<r + 1, m, itype, ttype>::value;
};
...
/** @brief index_position contains the zero-based index position of an index
 * type within a multi-index
 *
 * @note a multi-index represents as tuple of single indexes of type
 * boost::numeric::ublas::index::index_type
 *
 ...
 * @endcode
 *
 * @returns value returns 0 and N-1 if index_type is found, N otherwise where N
 * is tuple_size_v<tuple_type>.
 *
 * @tparam index_type type of index
 * @tparam tuple_type type of std::tuple that is searched for index
 */
template <class index_type, class tuple_type> struct index_position {
  static constexpr auto value =
    detail::index_position_impl<0ul,
                                std::tuple_size<tuple_type>::value,
                                std::decay_t<index_type>,
                                std::decay_t<tuple_type>>::value;
};
...
template <std::size_t r, std::size_t m> struct index_position_pairs_impl {
  template <class array_type, class tuple_left, class tuple_right>
  static constexpr void run(array_type& out,
                            tuple_left const& lhs,
                            tuple_right const& rhs,
                            std::size_t p)
  {
...
template <std::size_t m> struct index_position_pairs_impl<m, m> {
  template <class array_type, class tuple_left, class tuple_right>
  static constexpr void run([[maybe_unused]] array_type& out,
...
template <std::size_t r> struct index_position_pairs_impl<r, 0> {
  template <class array_type, class tuple_left, class tuple_right>
  static constexpr void run([[maybe_unused]] array_type& a,
                            [[maybe_unused]] tuple_left const& t1,
                            [[maybe_unused]] tuple_right const& t2,
...
/** @brief index_position_pairs returns zero-based index positions of matching
 * indexes of two multi-indexes
 *
 * @note a multi-index represents as tuple of single indexes of type
 * boost::numeric::ublas::index::index_type
 *
 * @code auto pairs = index_position_pairs(std::make_tuple(_a,_b),
 * std::make_tuple(_b,_c));
 * @endcode
 *
 * @returns a std::array instance containing index position pairs of type
...
constexpr auto array_to_vector_impl(
  array_type const& array, [[maybe_unused]] std::index_sequence<R...> sq)
{
...
/** @brief array_to_vector converts a std::array of zero-based index position
 * pairs into two std::vector of one-based index positions
 *
 * @code auto two_vectors = array_to_vector(std::make_array (
