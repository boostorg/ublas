...
void ttt(SizeType const k,
         ...
         SizeType const s,
         SizeType const q,
         SizeType const* const phia,
         SizeType const* const phib,
         PointerOut c,
         SizeType const* const nc,
         SizeType const* const wc,
         PointerIn1 a,
         SizeType const* const na,
         SizeType const* const wa,
         PointerIn2 b,
         SizeType const* const nb,
         SizeType const* const wb)
{
  if (k < r) {
    ...
    for (size_t ia = 0u; ia < na[phia[k - s] - 1];
         a += wa[phia[k - s] - 1], b += wb[phib[k - r] - 1], ++ia)
      ttt(k + 1, r, s, q, phia, phib, c, nc, wc, a, na, wa, b, nb, wb);
  }
  else {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
...
/** @brief Computes the tensor-times-matrix product for the contraction mode m >
 * 0
 *
 * Implements C[i1,i2,...,im-1,j,im+1,...,ip] = sum(A[i1,i2,...,im,...,ip] *
 * B[j,im])
 *
...
/** @brief Computes the tensor-times-vector product for the contraction mode m >
 * 0
 *
 * Implements C[i1,i2,...,im-1,im+1,...,ip] = sum(A[i1,i2,...,im,...,ip] *
 * b[im])
 *
         ...
         PointerIn2 b)
{
  if (r == m) {
    ...
    for (auto i0 = 0u; i0 < na[0]; c += wc[0], a += wa[0], ++i0) {
      ...
      auto a1 = a;
      auto b1 = b;
      for (auto im = 0u; im < na[m]; a1 += wa[m], ++b1, ++im) *c1 += *a1 * *b1;
    }
  }
  else {
          ...
          PointerIn2 b)
{
  if (r > 1) {
    ...
    for (auto i1 = 0u; i1 < na[1]; c += wc[0], a += wa[1], ++i1) {
      ...
      auto a1 = a;
      auto b1 = b;
      for (auto i0 = 0u; i0 < na[0]; a1 += wa[0], ++b1, ++i0) *c1 += *a1 * *b1;
    }
  }
}
...
void mtv(SizeType const m,
         PointerOut c,
         [[maybe_unused]] SizeType const* const nc,
         SizeType const* const wc,
         PointerIn1 a,
         SizeType const* const na,
         SizeType const* const wa,
         PointerIn2 b)
{
  // decides whether matrix multiplied with vector or vector multiplied with
  // matrix
...
value_t inner(SizeType const r,
...
{
  if (r == 0)
    for (auto i0 = 0u; i0 < n[0]; a += wa[0], b += wb[0], ++i0) v += *a * *b;
  else
 ...
 *
 *
 * @param[in]  pa number of dimensions (rank) of the first input tensor A with
 * pa > 0
 *
           ...
           SizeType const* const wb)
{
  if (rb > 1)
    ...
    outer_2x2(pa, c, nc, wc, a, na, wa, b, nb, wb);   // assert(ra==1 && rb==1
                                                      // && rc==3);
}
 ...
 *
 * Implements
 *   C[i1,i2,...,im-1,im+1,...,ip] = sum(A[i1,i2,...,im,...,ip] * b[im]) for m>1
 * and C[i2,...,ip]                  = sum(A[i1,...,ip]           * b[i1]) for
 * m=1
 *
 * @note calls detail::ttv, detail::ttv0 or detail::mtv
 *
 * @param[in]  m  contraction mode with 0 < m <= p
 * @param[in]  p  number of dimensions (rank) of the first input tensor with p >
 * 0
...
void ttv(SizeType const m,
         ...
         SizeType const* const wb)
{
  static_assert(std::is_pointer<PointerOut>::value &
                  std::is_pointer<PointerIn1>::value &
                  std::is_pointer<PointerIn2>::value,
                "Static error in boost::numeric::ublas::ttv: Argument types "
                "for pointers are not pointer types.");

  if (m == 0)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttv: Contraction mode must be greater "
      "than zero.");

  if (p < m)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttv: Rank must be greater equal the "
      "modus.");

  if (p == 0)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttv: Rank must be greater than zero.");

  if (c == nullptr || a == nullptr || b == nullptr)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttv: Pointers shall not be null "
      "pointers.");

  for (auto i = 0u; i < m - 1; ++i)
    if (na[i] != nc[i])
      throw std::length_error(
        "Error in boost::numeric::ublas::ttv: Extents (except of dimension "
        "mode) of A and C must be equal.");
  ...
  if (na[m - 1] != max)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttv: Extent of dimension mode of A and "
      "b must be equal.");
 ...
 *
 * Implements
 *   C[i1,i2,...,im-1,j,im+1,...,ip] = sum(A[i1,i2,...,im,...,ip] * B[j,im]) for
 * m>1 and C[j,i2,...,ip]                  = sum(A[i1,i2,...,ip]        *
 * B[j,i1]) for m=1
 *
         ...
         SizeType const* const wb)
{
  static_assert(std::is_pointer<PointerOut>::value &
                  std::is_pointer<PointerIn1>::value &
                  std::is_pointer<PointerIn2>::value,
                "Static error in boost::numeric::ublas::ttm: Argument types "
                "for pointers are not pointer types.");

  if (m == 0)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm: Contraction mode must be greater "
      "than zero.");

  if (p < m)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm: Rank must be greater equal than "
      "the specified mode.");

  if (p == 0)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm:Rank must be greater than zero.");

  if (c == nullptr || a == nullptr || b == nullptr)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm: Pointers shall not be null "
      "pointers.");

  for (auto i = 0u; i < m - 1; ++i)
    if (na[i] != nc[i])
      throw std::length_error(
        "Error in boost::numeric::ublas::ttm: Extents (except of dimension "
        "mode) of A and C must be equal.");
  ...
  if (na[m - 1] != nb[1])
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm: 2nd Extent of B and M-th Extent of "
      "A must be the equal.");

  if (nc[m - 1] != nb[0])
    throw std::length_error(
      "Error in boost::numeric::ublas::ttm: 1nd Extent of B and M-th Extent of "
      "C must be the equal.");
 ...
 * @param[in]  pa number of dimensions (rank) of the first input tensor a with
 * pa > 0
 * @param[in]  pb number of dimensions (rank) of the second input tensor b with
 * pb > 0
 * @param[in]  q  number of contraction dimensions with pa >= q and pb >= q and
 * q >= 0
         ...
         SizeType const* const wb)
{
  ...
  if (pa == 0 || pb == 0)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttt: tensor order must be greater "
      "zero.");

  if (q > pa && q > pb)
    throw std::length_error(
      "Error in boost::numeric::ublas::ttt: number of contraction must be "
      "smaller than or equal to the tensor order.");
  ...
  for (auto i = 0ul; i < r; ++i)
    if (na[phia[i] - 1] != nc[i])
      throw std::length_error(
        "Error in boost::numeric::ublas::ttt: dimensions of lhs and res tensor "
        "not correct.");

  for (auto i = 0ul; i < s; ++i)
    if (nb[phib[i] - 1] != nc[r + i])
      throw std::length_error(
        "Error in boost::numeric::ublas::ttt: dimensions of rhs and res not "
        "correct.");

  for (auto i = 0ul; i < q; ++i)
    if (nb[phib[s + i] - 1] != na[phia[r + i] - 1])
      throw std::length_error(
        "Error in boost::numeric::ublas::ttt: dimensions of lhs and rhs not "
        "correct.");


  if (q == 0ul)
    detail::recursive::outer(
      SizeType{0}, r, s, phia, phib, c, nc, wc, a, na, wa, b, nb, wb);
  else
    detail::recursive::ttt(
      SizeType{0}, r, s, q, phia, phib, c, nc, wc, a, na, wa, b, nb, wb);
}
         ...
         SizeType const* const wb)
{
  ...
  if (q == 0ul)
    detail::recursive::outer(
      pa, pc - 1, c, nc, wc, pa - 1, a, na, wa, pb - 1, b, nb, wb);
  else if (r == 0ul && s == 0ul)
    *c = detail::recursive::inner(q - 1, na, a, wa, b, wb, value_type(0));
  else
    detail::recursive::ttt(
      SizeType{0}, r, s, q, c, nc, wc, a, na, wa, b, nb, wb);
}
...
auto inner(const SizeType p,
           ...
           value_t v)
{
  static_assert(std::is_pointer<PointerIn1>::value &&
                  std::is_pointer<PointerIn2>::value,
                "Static error in boost::numeric::ublas::inner: Argument types "
                "for pointers must be pointer types.");
  if (p < 2)
    throw std::length_error(
      "Error in boost::numeric::ublas::inner: Rank must be greater than zero.");
  if (a == nullptr || b == nullptr)
    throw std::length_error(
      "Error in boost::numeric::ublas::inner: Pointers shall not be null "
      "pointers.");
           ...
           SizeType const* const wb)
{
  static_assert(std::is_pointer<PointerIn1>::value &
                ...
                "Static error in boost::numeric::ublas::outer: argument types "
                "for pointers must be pointer types.");
  if (pa < 2u || pb < 2u)
    throw std::length_error(
      "Error in boost::numeric::ublas::outer: number of extents of lhs and rhs "
      "tensor must be equal or greater than two.");
  if ((pa + pb) != pc)
    throw std::length_error(
      "Error in boost::numeric::ublas::outer: number of extents of lhs plus "
      "rhs tensor must be equal to the number of extents of C.");
  if (a == nullptr || b == nullptr || c == nullptr)
    throw std::length_error(
      "Error in boost::numeric::ublas::outer: pointers shall not be null "
      "pointers.");
