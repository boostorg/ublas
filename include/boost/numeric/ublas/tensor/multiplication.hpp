         ...
         PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
  if(k < r) {
    ...
    for(SizeType ic = 0u; ic < nc[k]; a += wa[phia[k]-1], c += wc[k], ++ic) {
      ttt(k+1, r, s, q,  phia,phib,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else if(k < r+s) {
    assert(nc[k] == nb[phib[k-r]-1]);
    for(SizeType ic = 0u; ic < nc[k]; b += wb[phib[k-r]-1], c += wc[k], ++ic) {
      ttt(k+1, r, s, q,  phia, phib,    c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else if(k < r+s+q-1) {
    assert(na[phia[k-s]-1] == nb[phib[k-r]-1]);
    for(SizeType ia = 0u; ia < na[phia[k-s]-1]; a += wa[phia[k-s]-1], b += wb[phib[k-r]-1], ++ia) {
      ttt(k+1, r, s, q,  phia, phib,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else {
         ...
         PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
  if(k < r) {
    assert(nc[k] == na[k]);
    for(auto ic = 0ul; ic < nc[k]; a += wa[k], c += wc[k], ++ic) {
      ttt(k+1, r, s, q,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else if(k < r+s) {
    assert(nc[k] == nb[k-r]);
    for(auto ic = 0ul; ic < nc[k]; b += wb[k-r], c += wc[k], ++ic) {
      ttt(k+1, r, s, q,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else if(k < r+s+q-1) {
    assert(na[k-s] == nb[k-r]);
    for(auto ia = 0ul; ia < na[k-s]; a += wa[k-s], b += wb[k-r], ++ia) {
      ttt(k+1, r, s, q,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else {
           ...
           PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
  if(k < r) {
    assert(nc[k] == na[phia[k]-1]);
    for(auto ic = 0ul; ic < nc[k]; a += wa[phia[k]-1], c += wc[k], ++ic){
      outer(k+1, r, s,   phia,phib,  c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else if(k < r+s-1) {
    assert(nc[k] == nb[phib[k-r]-1]);
    for(auto ic = 0ul; ic < nc[k]; b += wb[phib[k-r]-1], c += wc[k], ++ic){
      outer(k+1, r, s, phia, phib,    c, nc, wc,   a, na, wa,   b, nb, wb);
    }
  }
  else {
    assert(nc[k] == nb[phib[k-r]-1]);
    for(auto ic = 0u; ic < nc[k]; b += wb[phib[k-r]-1], c += wc[k], ++ic){
      *c = *a * *b;
    }
  }
}
