...
[[nodiscard]] inline constexpr bool operator==(LStrides const& lhs,
                                               RStrides const& rhs) noexcept
{
  static_assert(std::is_same_v<typename LStrides::value_type,
                               typename RStrides::value_type>,
                "boost::numeric::ublas::operator==(LStrides,RStrides) : LHS "
 ...
 *
 * @code auto j = access(std::vector{3,4,5}, strides{shape{4,2,3},first_order});
 * @endcode
 *
...
[[nodiscard]] inline constexpr auto access(
  std::vector<typename Stride::value_type> const& i, Stride const& w)
{
  static_assert(is_strides_v<Stride>,
                "boost::numeric::ublas::detail::access() : invalid type, type "
                "should be a strides");
...
[[nodiscard]] inline constexpr auto access(Stride const& w, Indices... is)
{
  static_assert(is_strides_v<Stride>,
                "boost::numeric::ublas::detail::access() : invalid type, type "
                "should be a strides");
