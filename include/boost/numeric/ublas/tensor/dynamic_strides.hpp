...
public:
  ...
  static_assert(std::numeric_limits<typename base_type::value_type>::is_integer,
                "Static error in boost::numeric::ublas::basic_strides: type "
                "must be of type integer.");
  static_assert(!std::numeric_limits<typename base_type::value_type>::is_signed,
                "Static error in boost::numeric::ublas::basic_strides: type "
                "must be of type unsigned integer.");
  static_assert(std::is_same<__layout, layout::first_order>::value ||
                  std::is_same<__layout, layout::last_order>::value,
                "Static error in boost::numeric::ublas::basic_strides: layout "
  ...
  /** @brief Constructs basic_strides from basic_extents for the first- and
   * last-order storage formats
   *
   * @code auto strides = basic_strides<unsigned>(
    ...
    : _base(s.size(), 1)
  {
    static_assert(is_extents_v<ExtentsType>,
                  "boost::numeric::ublas::basic_strides(ExtentsType const&) : "
    ...
    if (!valid(s))
      throw std::runtime_error(
        "Error in boost::numeric::ublas::basic_strides(ExtentsType const&) : "
        "shape is not valid.");
  ...
  basic_strides& operator=(basic_strides&& other) noexcept(
  ...
  basic_strides& operator=(basic_strides const& other) noexcept(
    std::is_nothrow_swappable_v<base_type>)
  {
  ...
  friend void swap(basic_strides& lhs, basic_strides& rhs) noexcept(
    std::is_nothrow_swappable_v<base_type>)
  {
