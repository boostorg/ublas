...
template <class T> class tensor_core;
  ...
  if constexpr (!std::is_same_v<T1, T2>) {
    ...
    if (!boost::numeric::ublas::detail::all_extents_equal(lhs, e)) {
      throw std::runtime_error(
        "operator+() : LHS tensor and RHS tensor should have equal extents");
    }
  }

  return boost::numeric::ublas::detail::make_binary_tensor_expression<T1>(
    lhs(), rhs(), [](auto const& l, auto const& r) { return l + r; });
}
template <class T1, class T2, class L, class R>
constexpr auto operator-(
  ...
  boost::numeric::ublas::detail::tensor_expression<T2, R> const& rhs)
{
  ...
  return boost::numeric::ublas::detail::make_binary_tensor_expression<T1>(
    lhs(), rhs(), [](auto const& l, auto const& r) { return l - r; });
  //	return
  //boost::numeric::ublas::detail::make_lambda<T>([&lhs,&rhs](std::size_t i){
  //return lhs(i) - rhs(i);});
}
template <class T1, class T2, class L, class R>
constexpr auto operator*(
...
constexpr auto operator+(
  typename boost::numeric::ublas::tensor_core<T>::const_reference lhs,
  boost::numeric::ublas::detail::
    tensor_expression<boost::numeric::ublas::tensor_core<T>, R> const& rhs)
{
  using tensor_core_type = boost::numeric::ublas::tensor_core<T>;
  return boost::numeric::ublas::detail::make_unary_tensor_expression<
    tensor_core_type>(rhs(), [lhs](auto const& r) { return lhs + r; });
  // return boost::numeric::ublas::detail::make_lambda<T>(
  // [&lhs,&rhs](std::size_t i) {return lhs + rhs(i); } );
}
template <class T, class R>
constexpr auto operator-(
  typename boost::numeric::ublas::tensor_core<T>::const_reference lhs,
...
constexpr auto operator+(
  boost::numeric::ublas::detail::
    tensor_expression<boost::numeric::ublas::tensor_core<T>, L> const& lhs,
  typename boost::numeric::ublas::tensor_core<T>::const_reference rhs)
{
  using tensor_core_type = boost::numeric::ublas::tensor_core<T>;
  return boost::numeric::ublas::detail::make_unary_tensor_expression<
    tensor_core_type>(lhs(), [rhs](auto const& l) { return l + rhs; });
}
template <class T, class L>
constexpr auto operator-(
  boost::numeric::ublas::detail::
...
constexpr auto& operator+=(
  ...
  const boost::numeric::ublas::detail::
    tensor_expression<boost::numeric::ublas::tensor_core<T>, D>& expr)
{
  boost::numeric::ublas::detail::eval(
    lhs, expr(), [](auto& l, auto const& r) { l += r; });
  return lhs;
}
...
constexpr auto const& operator+(
  const boost::numeric::ublas::detail::tensor_expression<T, D>& lhs)
{
  return lhs;
}
...
template <class T, class D>
constexpr auto operator-(
  boost::numeric::ublas::detail::tensor_expression<T, D> const& lhs)
{
  return boost::numeric::ublas::detail::make_unary_tensor_expression<T>(
    lhs(), [](auto const& l) { return -l; });
}
...
template <class tensor_type_left,
          class tuple_type_left,
          class tensor_type_right,
          class tuple_type_right>
               ...
               std::pair<tensor_type_right const&, tuple_type_right> rhs)
{
  ...
  if constexpr (num_equal_ind == 0) {
  ...
  else if constexpr (num_equal_ind == std::tuple_size<tuple_type_left>::value &&
    ...
    return boost::numeric::ublas::prod(
      tensor_left, tensor_right, index_pairs.first, index_pairs.second);
  }
}
