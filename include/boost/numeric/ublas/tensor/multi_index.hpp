 ...
 *
 * Denotes an array of index_type types ::_a for 0<=K<=16 is used in
 * tensor::operator()
 */
template <std::size_t N> class multi_index {
public:
  multi_index() = delete;
    ...
    : _base{i(), is()...}
  {
    static_assert(
      sizeof...(is) + 1 == N,
      "Static assert in boost::numeric::ublas::multi_index: number of "
      "constructor arguments is not equal to the template parameter.");

    static_assert(
      valid_multi_index<std::tuple<index::index_type<I>, indexes...>>::value,
      "Static assert in boost::numeric::ublas::multi_index: indexes occur "
      "twice in multi-index.");
  }
...
constexpr auto get(multi_index<N> const& m)
{
  return std::get<K>(m.base());
}
  ...
  for (auto i = 0u; i < N; ++i)
    for (auto j = 0u; j < M; ++j)
      if (lhs.at(i) == rhs.at(j) &&
          lhs.at(i) != boost::numeric::ublas::index::_()) {
