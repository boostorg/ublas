...
template <class V, class A> struct storage_traits<std::vector<V, A>> {
  ...
  template <class U>
  using rebind =
    std::vector<U, typename std::allocator_traits<A>::template rebind_alloc<U>>;
};


template <class V, std::size_t N> struct storage_traits<std::array<V, N>> {
  ...
  template <class U> using rebind = std::array<U, N>;

  template <std::size_t M> using rebind_size = std::array<V, M>;
};
