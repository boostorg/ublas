...
template <typename T> struct is_valid_tensor : std::false_type {
};

template <typename T> struct is_valid_tensor<tensor_core<T>> : std::true_type {
};

template <typename T>
inline static constexpr bool is_valid_tensor_v = is_valid_tensor<T>::value;

template <typename E, typename A, typename ValueType> struct rebind_storage {
...
template <typename A, std::size_t N>
using rebind_static_storage_helper_t = typename rebind_static_storage_helper<
  A,
  N,
  typename storage_traits<A>::resizable_tag>::type;

}   // namespace detail

template <typename ValueType, typename A, typename T, T... Ns>
struct rebind_storage<basic_static_extents<T, Ns...>, A, ValueType>
  : std::conditional<std::is_same_v<typename storage_traits<A>::resizable_tag,
