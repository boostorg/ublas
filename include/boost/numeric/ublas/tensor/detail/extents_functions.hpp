...
constexpr auto push_back(basic_static_extents<T, N...>)
  -> basic_static_extents<T, N..., E>;

template <typename T, T E, T... N>
constexpr auto push_front(basic_static_extents<T, N...>)
  -> basic_static_extents<T, E, N...>;
  ...
  basic_static_extents<T, N...> num = basic_static_extents<T>{})
{
  ...
  if constexpr (sizeof...(E) == 0ul) {
    ...
    if constexpr (E0 == T(1)) {
      ...
      return squeeze_impl_remove_one(basic_static_extents<T, E...>{},
                                     n_num_list);
    }
  }
}
...
constexpr auto squeeze_impl(basic_static_extents<T, E...> const& e)
{
  ...
  // squeeze
  if constexpr (decltype(one_free_static_extents)::_size !=
                extents_type::_size) {
    ...
    if constexpr (decltype(one_free_static_extents)::_size == size_type(0)) {
      ...
      if constexpr (extents_type::at(1) == value_type(1)) {
        ...
        return decltype(
          push_front<T, value_type(1)>(one_free_static_extents)){};
      }
    }
    else {
...
template <class T> inline constexpr auto squeeze_impl(basic_extents<T> const& e)
{
  ...
  auto not_one = [](auto const& el) { return el != value_type(1); };
...
inline auto squeeze_impl(basic_fixed_rank_extents<T, N> const& e)
{
  if constexpr (N <= 2) {
    return e;
  }
...
[[nodiscard]] inline constexpr bool valid(ExtentsType const& e)
{
  static_assert(
    is_extents_v<ExtentsType>,
    "boost::numeric::ublas::valid() : invalid type, type should be an extents");
...
template <class T>[[nodiscard]] inline std::string to_string(T const& e)
{
  using value_type = typename T::value_type;

  static_assert(is_extents_v<T> || is_strides_v<T>,
                "boost::numeric::ublas::to_string() : invalid type, type "
                "should be an extents or a strides");
  ...
  std::copy(
    e.begin(), e.end() - 1, std::ostream_iterator<value_type>(ss, ", "));
 ...
 */
template <class ExtentsType>
[[nodiscard]] inline constexpr bool is_scalar(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::is_scalar() : invalid type, type "
                "should be an extents");
 ...
 */
template <class ExtentsType>
[[nodiscard]] inline constexpr bool is_vector(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::is_vector() : invalid type, type "
                "should be an extents");
 ...
 */
template <class ExtentsType>
[[nodiscard]] inline constexpr bool is_matrix(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::is_matrix() : invalid type, type "
                "should be an extents");
  ...
  return (e.size() >= 2u) &&
         std::all_of(e.begin(), e.begin() + 2, greater_one) &&
         std::all_of(e.begin() + 2, e.end(), equal_one);
}
 ...
 */
template <class ExtentsType>
[[nodiscard]] inline constexpr bool is_tensor(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::is_tensor() : invalid type, type "
                "should be an extents");
 ...
 */
template <class ExtentsType>
[[nodiscard]] inline auto squeeze(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::squeeze() : invalid type, type should "
                "be an extents");
...
template <class ExtentsType>
[[nodiscard]] inline constexpr auto product(ExtentsType const& e)
{
  static_assert(is_extents_v<ExtentsType>,
                "boost::numeric::ublas::product() : invalid type, type should "
                "be an extents");
...
[[nodiscard]] inline constexpr bool operator==(LExtents const& lhs,
                                               RExtents const& rhs) noexcept
{
  static_assert(std::is_same_v<typename LExtents::value_type,
                               typename RExtents::value_type>,
                "boost::numeric::ublas::operator==(LExtents, RExtents) : LHS "
                "value type should be same as RHS value type");
