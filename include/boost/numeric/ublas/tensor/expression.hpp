...
struct tensor_expression : public ublas_expression<E> {
...
struct binary_tensor_expression
  ...
  explicit constexpr binary_tensor_expression(expression_type_left const& l,
                                              expression_type_right const& r,
                                              binary_operation o)
    : el(l)
    , er(r)
    , op(o)
  ...
  constexpr binary_tensor_expression& operator=(
    binary_tensor_expression&& l) noexcept
  {
    el(l.el);
    er(l.er);
    op(l.op);
  ...
  binary_tensor_expression& operator            =(
    binary_tensor_expression const& l) noexcept = delete;

  ~binary_tensor_expression() = default;

  inline decltype(auto) operator()(size_type i) const
  {
    return op(el(i), er(i));
  }
...
auto make_binary_tensor_expression(tensor_expression<T1, EL> const& el,
                                   tensor_expression<T2, ER> const& er,
                                   OP op)
{
  static_assert(
    std::is_same_v<typename T1::value_type, typename T2::value_type>,
    "boost::numeric::ublas::make_binary_tensor_expression(T1,T2) : LHS tensor "
    "and RHS tensor should have same value type");
  return binary_tensor_expression<T1, EL, ER, OP>(el(), er(), op);
}

template <class T, class EL, class ER, class OP>
auto make_binary_tensor_expression(matrix_expression<EL> const& el,
                                   tensor_expression<T, ER> const& er,
                                   OP op)
{
...
template <class T, class EL, class ER, class OP>
auto make_binary_tensor_expression(tensor_expression<T, EL> const& el,
                                   matrix_expression<ER> const& er,
                                   OP op)
{
  return binary_tensor_expression<T, EL, ER, OP>(el(), er(), op);
}
...
template <class T, class EL, class ER, class OP>
auto make_binary_tensor_expression(vector_expression<EL> const& el,
                                   tensor_expression<T, ER> const& er,
                                   OP op)
{
  return binary_tensor_expression<T, EL, ER, OP>(el(), er(), op);
...
struct unary_tensor_expression
  ...
  explicit constexpr unary_tensor_expression(E const& ee, OP o)
    : e(ee)
    , op(o)
  {
  }
  ...
  constexpr unary_tensor_expression& operator=(
    unary_tensor_expression&& l) noexcept
  {
    e(l.e);
    op(l.op);
  }

  unary_tensor_expression& operator            =(
    unary_tensor_expression const& l) noexcept = delete;
  ~unary_tensor_expression()                   = default;

  inline decltype(auto) operator()(size_type i) const { return op(e(i)); }
