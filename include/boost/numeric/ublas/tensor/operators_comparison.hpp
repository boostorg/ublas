...
[[nodiscard]] inline constexpr bool compare(tensor_core<T1> const& lhs,
                                            tensor_core<T2> const& rhs,
                                            BinaryPred pred)
{
  static_assert(std::is_same_v<typename tensor_core<T1>::value_type,
                               typename tensor_core<T2>::value_type>,
                "boost::numeric::ublas::detail::compare() : LHS and RHS both "
                "should have same value type");

  if (lhs.extents() != rhs.extents()) {
    if constexpr (!std::is_same<BinaryPred, std::equal_to<>>::value &&
                  !std::is_same<BinaryPred, std::not_equal_to<>>::value)
      throw std::runtime_error(
        "Error in boost::numeric::ublas::detail::compare: cannot compare "
        "tensors with different shapes.");
    else
      return false;
  }

  if constexpr (std::is_same<BinaryPred, std::greater<>>::value ||
                std::is_same<BinaryPred, std::less<>>::value)
    if (lhs.empty())
      return false;
...
[[nodiscard]] inline constexpr bool compare(tensor_core<T> const& rhs,
                                            UnaryPred pred)
{
...
[[nodiscard]] constexpr bool compare(tensor_expression<T1, L> const& lhs,
                                     tensor_expression<T2, R> const& rhs,
                                     BinaryPred pred)
{
  ...
  if constexpr (lhs_is_tensor && rhs_is_tensor)
    return compare(
      static_cast<T1 const&>(lhs), static_cast<T2 const&>(rhs), pred);
...
[[nodiscard]] constexpr bool compare(tensor_expression<T, D> const& expr,
                                     UnaryPred pred)
{
...
[[nodiscard]] inline constexpr auto operator!=(
  ...
  boost::numeric::ublas::detail::tensor_expression<T2, R> const& rhs)
{
  return boost::numeric::ublas::detail::compare(
...
[[nodiscard]] inline constexpr bool operator==(
  typename T::const_reference lhs,
  boost::numeric::ublas::detail::tensor_expression<T, D> const& rhs)
{
  return boost::numeric::ublas::detail::compare(
    rhs, [lhs](auto const& r) { return lhs == r; });
}
template <class T, class D>
[[nodiscard]] inline constexpr auto operator!=(
  typename T::const_reference lhs,
...
{
  return boost::numeric::ublas::detail::compare(
    lhs, [rhs](auto const& l) { return l == rhs; });
}
template <class T, class D>
[[nodiscard]] inline constexpr auto operator!=(
  boost::numeric::ublas::detail::tensor_expression<T, D> const& lhs,
