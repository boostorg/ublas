...
template <typename T, T... P> struct static_stride_list {
...
// @code using type = typename concat< static_stride_list<int, 1,2,3>,
// static_stride_list<int, 4,5,6> >::type @endcode
...
template <typename T, std::size_t N> struct make_sequence_of_ones;

template <typename T, std::size_t N>
using make_sequence_of_ones_t = typename make_sequence_of_ones<T, N>::type;

template <typename T, std::size_t N> struct make_sequence_of_ones {
  using type = concat_t<make_sequence_of_ones_t<T, N / 2>,
                        make_sequence_of_ones_t<T, N - N / 2>>;
};

template <typename T> struct make_sequence_of_ones<T, 0ul> {
  using type = static_stride_list<T>;
};
template <typename T> struct make_sequence_of_ones<T, 1ul> {
  ...
  [[maybe_unused]] static_stride_list<T, P...> e3)
{
  if constexpr (sizeof...(E) == 0ul) {
    ...
    if constexpr (std::is_same_v<layout::last_order, L>) {
      auto np = static_stride_list<
        T,
        P...,
        static_product_v<basic_static_extents<T, E..., E0>>>{};
      return make_static_strides_helper<L>(
        static_stride_list<T, E...>{}, n, np);
    }
    else {
      auto np = static_stride_list<
        T,
        P...,
        static_product_v<basic_static_extents<T, R..., E0>>>{};
      return make_static_strides_helper<L>(
        static_stride_list<T, E...>{}, n, np);
    }
  }
...
constexpr auto make_static_strides([
  [maybe_unused]] static_stride_list<T, E0, E...> e)
{
  using extents_type = typename static_stride_list<T, E0, E...>::extents_type;
  // checks if extents are vector or scalar
  if constexpr (!(static_traits::is_scalar_v<extents_type> ||
    ...
    if constexpr (sizeof...(E) == 0) {
      ...
      if constexpr (std::is_same_v<L, layout::first_order>) {
        using ret_type = decltype(
          make_static_strides_helper<L>(static_stride_list<T, E0, E...>{},
                                        static_stride_list<T>{},
                                        static_stride_list<T>{}));
        return impl::concat_t<static_stride_list<T, T(1)>, ret_type>{};
      }
      else {
        using ret_type =
          decltype(make_static_strides_helper<L>(static_stride_list<T, E...>{},
                                                 static_stride_list<T>{},
                                                 static_stride_list<T>{}));
...
template <typename Layout, typename T, T... E> struct strides_helper {
  using type =
    decltype(make_static_strides<Layout>(static_stride_list<T, E...>{}));
  static constexpr auto value = type::value;
};

template <typename Layout, typename T, T... E>
inline static constexpr auto strides_helper_v =
  strides_helper<Layout, T, E...>::value;
 ...
 *
 * @code basic_static_strides<basic_static_extents<4,1,2,3,4>,
 * layout::first_order> s @endcode
 *
...
public:
  ...
  constexpr basic_static_strides() noexcept
  {
    static_assert(_size == 0 ||
                    (static_traits::is_valid_v<extents_type> &&
                     (static_traits::is_vector_v<extents_type> ||
                      static_traits::is_scalar_v<extents_type> || _size >= 2)),
                  "Error in boost::numeric::ublas::basic_static_strides() : "
                  "Size can be 0 or Shape should be valid and shape can be "
                  "vector or shape can be scalar or size should be greater than"
                  " or equal to 2");
  }
  ...
  constexpr basic_static_strides(basic_static_strides const& other) noexcept =
    default;
  constexpr basic_static_strides(basic_static_strides&& other) noexcept =
    default;

  // default assign constructor
  constexpr basic_static_strides& operator      =(
    basic_static_strides const& other) noexcept = default;

  constexpr basic_static_strides& operator =(
    basic_static_strides&& other) noexcept = default;

  ~basic_static_strides() = default;

  /** @brief Returns ref to the std::array containing extents */
  [[nodiscard]] inline constexpr auto const& base() const noexcept
  {
    return m_data;
  }
...
private:
  static constexpr base_type const m_data{
    detail::strides_helper_v<layout_type, T, Extents...>};
};
