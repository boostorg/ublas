...
/** @brief Template class for storing tensor strides for iteration with runtime
 * variable size.
 *
...
public:
  ...
  static_assert(
    ...
    "Static error in boost::numeric::ublas::basic_fixed_rank_strides: type "
    "must be of type integer.");
  static_assert(
    !std::numeric_limits<value_type>::is_signed,
    "Static error in boost::numeric::ublas::basic_fixed_rank_strides: type "
    "must be of type unsigned integer.");
  static_assert(
    std::is_same<L, layout::first_order>::value ||
      std::is_same<L, layout::last_order>::value,
    "Static error in boost::numeric::ublas::basic_fixed_rank_strides: layout "
    "type must either first or last order");
  ...
  /** @brief Constructs basic_fixed_rank_strides from basic_extents for the
   * first- and last-order storage formats
   *
   * @code auto strides = basic_fixed_rank_strides<unsigned>(
   * basic_extents<std::size_t>{2,3,4} );
   *
   */
  template <typename ExtentsType> basic_fixed_rank_strides(ExtentsType const& s)
  {
    if (s.size() != size()) {
      throw std::length_error(
        "boost::numeric::ublas::basic_fixed_rank_strides(ExtentsType const&) : "
        "ExentsType size should be equal to the size of "
        "basic_fixed_rank_strides");
    }
    ...
    if (!valid(s))
      throw std::runtime_error(
        "Error in boost::numeric::ublas::basic_fixed_rank_strides(ExtentsType "
        "const&) : "
        "shape is not valid.");
  ...
  basic_fixed_rank_strides& operator=(
    basic_fixed_rank_strides const&
      other) noexcept(std::is_nothrow_swappable_v<base_type>)
  {
  ...
  friend void swap(basic_fixed_rank_strides& lhs,
                   basic_fixed_rank_strides&
                     rhs) noexcept(std::is_nothrow_swappable_v<base_type>)
  {
    std::swap(lhs._base, rhs._base);
  }

  [[nodiscard]] inline constexpr const_reference operator[](size_type p) const
  {
    return _base[p];
  }

  [[nodiscard]] inline constexpr const_pointer data() const
  {
    return _base.data();
  }

  [[nodiscard]] inline constexpr const_reference at(size_type p) const
  {
    return _base.at(p);
  }
  ...
  [[nodiscard]] inline constexpr reference back() { return _base.back(); }
