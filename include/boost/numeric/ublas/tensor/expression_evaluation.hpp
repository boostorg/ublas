...
template <typename ExtentType> class basic_extents;
...
template <class T, class E> struct has_tensor_types {
  static constexpr bool value = false;
};

template <class T> struct has_tensor_types<T, T> {
...
struct has_tensor_types<T, tensor_expression<T, D>> {
  static constexpr bool value =
    std::is_same<T, D>::value || has_tensor_types<T, D>::value;
};

template <class T, class EL, class ER, class OP>
struct has_tensor_types<T, binary_tensor_expression<T, EL, ER, OP>> {
  static constexpr bool value =
    std::is_same<T, EL>::value || std::is_same<T, ER>::value ||
    has_tensor_types<T, EL>::value || has_tensor_types<T, ER>::value;
};

template <class T, class E, class OP>
struct has_tensor_types<T, unary_tensor_expression<T, E, OP>> {
  static constexpr bool value =
    std::is_same<T, E>::value || has_tensor_types<T, E>::value;
};
...
[[nodiscard]] constexpr auto& retrieve_extents(
  tensor_core<TensorEngine> const& t)
{
  return t.extents();
}
 ...
 *
 * @note tensor_core expression must be a binary tree with at least one
 * tensor_core type
 *
 * @returns extents of the child expression if it is a tensor_core or extents of
 * one child of its child.
 */
template <class T, class D>
[[nodiscard]] constexpr auto& retrieve_extents(
  tensor_expression<T, D> const& expr)
{
  static_assert(detail::has_tensor_types<T, tensor_expression<T, D>>::value,
                "Error in boost::numeric::ublas::detail::retrieve_extents: "
                "Expression to evaluate should contain tensors.");
 ...
 * tensor_core type
 *
 * @returns extents of the (left and if necessary then right) child expression
 * if it is a tensor_core or extents of a child of its (left and if necessary
 * then right) child.
 */
template <class T, class EL, class ER, class OP>
[[nodiscard]] constexpr auto& retrieve_extents(
  binary_tensor_expression<T, EL, ER, OP> const& expr)
{
  static_assert(
    detail::has_tensor_types<T, binary_tensor_expression<T, EL, ER, OP>>::value,
    "Error in boost::numeric::ublas::detail::retrieve_extents: Expression to "
    "evaluate should contain tensors.");
...
template <class T, class E, class OP>
[[nodiscard]] constexpr auto& retrieve_extents(
  unary_tensor_expression<T, E, OP> const& expr)
{
  static_assert(
    detail::has_tensor_types<T, unary_tensor_expression<T, E, OP>>::value,
    "Error in boost::numeric::ublas::detail::retrieve_extents: Expression to "
    "evaluate should contain tensors.");
...
[[nodiscard]] inline constexpr auto all_extents_equal(
  tensor_core<TensorEngine> const& t, Extents const& extents)
{
  static_assert(is_extents_v<Extents>,
                "Error in boost::numeric::ublas::detail::all_extents_equal: "
                "extents passed should be of extents type.");

  return extents == t.extents();
}

template <class T, class D, class Extents>
[[nodiscard]] constexpr auto all_extents_equal(
  tensor_expression<T, D> const& expr, Extents const& extents)
{
  static_assert(is_extents_v<Extents>,
                "Error in boost::numeric::ublas::detail::all_extents_equal: "
                "extents passed should be of extents type.");
...
[[nodiscard]] constexpr auto all_extents_equal(
  binary_tensor_expression<T, EL, ER, OP> const& expr, Extents const& extents)
{
  static_assert(is_extents_v<Extents>,
                "Error in boost::numeric::ublas::detail::all_extents_equal: "
                "extents passed should be of extents type.");
  ...
  static_assert(
    detail::has_tensor_types<T, binary_tensor_expression<T, EL, ER, OP>>::value,
    "Error in boost::numeric::ublas::detail::all_extents_equal: Expression to "
    "evaluate should contain tensors.");
...
[[nodiscard]] constexpr auto all_extents_equal(
  unary_tensor_expression<T, E, OP> const& expr, Extents const& extents)
{
  static_assert(is_extents_v<Extents>,
                "Error in boost::numeric::ublas::detail::all_extents_equal: "
                "extents passed should be of extents type.");
 ...
 *
 * \note Checks if shape of the tensor_core matches those of all tensors within
 * the expression.
 */
template <class tensor_type, class derived_type>
void eval(tensor_type& lhs,
          tensor_expression<tensor_type, derived_type> const& expr)
{
  if constexpr (detail::has_tensor_types<
                  tensor_type,
                  tensor_expression<tensor_type, derived_type>>::value)
...
#pragma omp parallel for
  for (auto i = 0u; i < lhs.size(); ++i) lhs(i) = expr()(i);
}
...
template <typename tensor_type,
          typename other_tensor_type,
          typename derived_type>
void eval(tensor_type& lhs,
          tensor_expression<other_tensor_type, derived_type> const& expr)
{
  static_assert(
    is_valid_tensor_v<tensor_type> && is_valid_tensor_v<other_tensor_type>,
    "boost::numeric::ublas::detail::eval(tensor_type&, "
    "tensor_expression<other_tensor_type, derived_type> const&) : "
    "tensor_type and tensor_expresssion should be a valid tensor type");

  static_assert(
    std::is_same_v<typename tensor_type::value_type,
                   typename other_tensor_type::value_type>,
    "boost::numeric::ublas::detail::eval(tensor_type&, "
    "tensor_expression<other_tensor_type, derived_type> const&) : "
...
/** @brief Evaluates expression for a tensor_core
 *
 * Applies a unary function to the results of the expressions before the
 * assignment. Usually applied needed for unary operators such as A += C;
 *
 * \note Checks if shape of the tensor_core matches those of all tensors within
 * the expression.
 */
...
#pragma omp parallel for
  for (auto i = 0u; i < lhs.size(); ++i) fn(lhs(i), expr()(i));
}
...
{
#pragma omp parallel for
  for (auto i = 0u; i < lhs.size(); ++i) fn(lhs(i));
}
