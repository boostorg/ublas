...
constexpr auto extents_result_tensor_times_vector([[maybe_unused]] E const& e)
{
  static_assert(is_static_rank<E>::value,
                "boost::numeric::ublas::extents_result_tensor_times_vector() : "
...
constexpr auto extents_result_tensor_times_vector(basic_extents<T> const& e)
{
  ...
  return dynamic_extents<>{typename dynamic_extents<>::base_type(
    std::max(size_type(e.size() - 1), size_type(2)), 1)};
}
...
{
  static_assert(is_static_rank<E>::value,
                "boost::numeric::ublas::extents_result_tensor_times_matrix() : "
                "invalid type, type should be an extents");
...
{
  return dynamic_extents<>(typename dynamic_extents<>::base_type(
    e1.size() + e2.size(), typename E1::value_type(1)));
}
...
auto extents_result_tensor_prod(E1 const& e1,
                                E2 const& e2,
                                A1 const& a,
                                [[maybe_unused]] A2 const& ta)
{
  auto const size = e1.size() + e2.size() - 2 * a.size();
  return dynamic_extents<>(typename dynamic_extents<>::base_type(
    std::max(size, std::size_t(2)), typename E1::value_type(1)));
}
...
inline decltype(auto) prod(
  ...
  matrix<typename tensor_core<TensorEngine>::value_type,
         typename tensor_core<TensorEngine>::layout_type,
         A> const& b,
  const std::size_t m)
{
  using tensor_type          = tensor_core<TensorEngine>;
  ...
  static_assert(
    std::is_same_v<typename tensor_core<TensorEngine>::resizable_tag,
 ...
 * input tensor a can be of type std::vector<std::size_t> or
 * std::array<std::size_t,N>
 * @param[in]  phib one-based permutation tuple of length q for the second
 * input tensor b can be of type std::vector<std::size_t> or
 * std::array<std::size_t,N>
 * @result     tensor with order r+s
 */
template <typename TensorEngine1,
          typename TensorEngine2,
          typename PermuType,
          std::enable_if_t<
...
inline decltype(auto) prod(tensor_core<TensorEngine1> const& a,
                           ...
                           PermuType const& phia,
                           PermuType const& phib)
{
  ...
  static_assert(
    std::is_same_v<typename tensor_core<TensorEngine1>::resizable_tag,
                   typename tensor_core<TensorEngine2>::resizable_tag> &&
      std::is_same_v<typename tensor_core<TensorEngine1>::resizable_tag,
                     storage_resizable_container_tag>,
    "error in boost::numeric::ublas::prod(tensor_core const&, tensor_core "
    "const&, "
    "PermuType const&, PermuType const& ): "
    "Both the tensor storage should have the same type of storage and both "
    "should be resizable");

  static_assert(
    !(is_static<extents_type_1>::value &&
      is_static<typename tensor_core<TensorEngine2>::extents_type>::value),
    "error in boost::numeric::ublas::prod(tensor_core< TensorEngine1 > const&, "
    "tensor_core< TensorEngine2 > const&, "
    "PermuType const&, PermuType const&): "
  ...
  if (pa == 0ul)
    throw std::runtime_error(
      "error in ublas::prod: order of left-hand side tensor must be greater "
      "than 0.");
  if (pb == 0ul)
    throw std::runtime_error(
      "error in ublas::prod: order of right-hand side tensor must be greater "
      "than 0.");
  if (pa < q)
    throw std::runtime_error(
  ...
  if (pb < phib.size())
    throw std::runtime_error(
      "error in ublas::prod: permutation tuple for the right-hand side tensor "
      "cannot be greater than the corresponding order.");
...
inline decltype(auto) inner_prod(tensor_core<TensorEngine1> const& a,
                                 tensor_core<TensorEngine2> const& b)
{
  ...
  static_assert(
    std::is_same_v<value_type, typename tensor_core<TensorEngine2>::value_type>,
    "error in boost::numeric::ublas::inner_prod(tensor_core< TensorEngine1 > "
    "const&, tensor_core< TensorEngine2 > const&): "
    "Both the tensor should have same value_type");

  if (a.rank() != b.rank())
    throw std::length_error(
      "error in boost::numeric::ublas::inner_prod: Rank of both the tensors "
      "must be the same.");

  if (a.empty() || b.empty())
    throw std::length_error(
      "error in boost::numeric::ublas::inner_prod: Tensors should not be "
      "empty.");

  if (a.extents() != b.extents())
    throw std::length_error(
      "error in boost::numeric::ublas::inner_prod: Tensor extents should be "
      "the same.");
...
template <typename TensorEngine1,
          ...
          std::enable_if_t<
            !(is_static_v<typename tensor_core<TensorEngine1>::extents_type> &&
              is_static_v<typename tensor_core<TensorEngine2>::extents_type>),
            int> = 0>
inline decltype(auto) outer_prod(tensor_core<TensorEngine1> const& a,
                                 tensor_core<TensorEngine2> const& b)
{
  using tensor_type = tensor_core<TensorEngine1>;
  using value_type  = typename tensor_type::value_type;
  ...
  static_assert(
    std::is_same_v<typename tensor_core<TensorEngine1>::resizable_tag,
      ...
      std::is_same_v<typename tensor_core<TensorEngine1>::resizable_tag,
                     storage_resizable_container_tag>,
    "error in boost::numeric::ublas::outer_prod(tensor_core const&, "
    "tensor_core const&): "
    "Both the tensor storage should have the same type of storage and both "
    "should be resizable");
  ...
  static_assert(
    std::is_same_v<value_type, typename tensor_core<TensorEngine2>::value_type>,
    "error in boost::numeric::ublas::outer_prod(tensor_core< TensorEngine1 > "
    "const&, tensor_core< TensorEngine2 > const&): "
    "Both the tensor should have same value_type");
  ...
  for (auto i = 0u; i < a.rank(); ++i) nc.at(i) = a_extents.at(i);

  for (auto i = 0u; i < b.rank(); ++i) nc.at(a.rank() + i) = b_extents.at(i);
...
inline decltype(auto) trans(tensor_core<TensorEngine> const& a,
                            std::vector<std::size_t> const& tau)
{
  ...
  static_assert(!is_static_rank_v<extents_type>,
                "error in boost::numeric::ublas::trans(tensor_core< "
                "TensorEngine > const &a, "
                "std::vector<std::size_t> const &tau): "
                "Tensor should have dynamic extents");

  using t_engine =
    tensor_engine<extents_type,
                  layout_type,
...
template <typename TensorEngine,
          class D,
          std::enable_if_t<detail::is_complex_v<
                             typename tensor_core<TensorEngine>::value_type>,
                           int> = 0>
auto conj(detail::tensor_expression<tensor_core<TensorEngine>, D> const& expr)
{
  return detail::make_unary_tensor_expression<tensor_core<TensorEngine>>(
...
auto conj(detail::tensor_expression<T, D> const& expr)
{
  ...
  std::transform(
    a.begin(), a.end(), c.begin(), [](auto const& l) { return std::conj(l); });
...
auto real(detail::tensor_expression<tensor_core<TensorEngine>, D> const& expr)
{
  ...
  std::transform(
    a.begin(), a.end(), c.begin(), [](auto const& l) { return std::real(l); });
...
auto imag(detail::tensor_expression<tensor_core<TensorEngine>, D> const& expr)
{
  ...
  std::transform(
    a.begin(), a.end(), c.begin(), [](auto const& l) { return std::imag(l); });
...
inline constexpr auto extents_result_tensor_times_vector(
  ...
  [[maybe_unused]] basic_static_extents<T, R...> e3 = basic_static_extents<T>{})
{
  if constexpr (I != M - 1) {
    return extents_result_tensor_times_vector<M, I + 1>(
      basic_static_extents<T, E...>{},
      basic_static_extents<T, OtherE...>{},
      basic_static_extents<T, R..., E0>{});
  }
  else {
    return extents_result_tensor_times_vector<M, I + 1>(
      basic_static_extents<T, E...>{},
      basic_static_extents<T, O0, OtherE...>{},
      basic_static_extents<T, R...>{});
  }
}
...
inline constexpr auto extents_result_tensor_times_vector(
  basic_static_extents<T, E0, E...> const& e)
{
  using size_type = typename basic_static_extents<T>::size_type;
  auto ones       = typename impl::make_sequence_of_ones_t<
    T,
    std::max(size_type(2), sizeof...(E))>::extents_type{};
  return extents_result_tensor_times_vector<M, 0>(e, ones);
}

template <size_t I, size_t NE, typename T, T E0, T... E, T... OtherE>
inline constexpr auto static_extents_set_at(
  [[maybe_unused]] basic_static_extents<T, E0, E...> const& e1,
  [[maybe_unused]] basic_static_extents<T, OtherE...> e2 =
    basic_static_extents<T>{})
{
  static_assert(
    I < sizeof...(E) + 1,
    "boost::numeric::ublas::detail::static_extents_set_at(): out of bound");
  if constexpr (sizeof...(E) == 0) {
    ...
    if constexpr (I == 0) {
      ...
      return static_extents_set_at<I - 1, NE>(
        basic_static_extents<T, E...>{},
        basic_static_extents<T, OtherE..., E0>{});
    }
  }
}
...
inline decltype(auto) prod(
  tensor_core<TensorType> const& a,
  vector<typename tensor_core<TensorType>::value_type, A> const& b)
{
  ...
  ttv(M,
      p,
      c.data(),
      c_static_extents.data(),
      c_static_strides.data(),
      a.data(),
      a_static_extents.data(),
      a_static_strides.data(),
      bb,
      nb.data(),
      nb.data());
...
inline decltype(auto) prod(tensor_core<TensorType> const& a,
                           matrix<typename tensor_core<TensorType>::value_type,
                                  typename tensor_core<TensorType>::layout_type,
                                  A> const& b)
{
  using tensor_type          = tensor_core<TensorType>;
  using extents_type         = typename tensor_type::extents_type;
