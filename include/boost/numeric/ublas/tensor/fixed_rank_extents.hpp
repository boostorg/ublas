...
public:
  ...
  static_assert(
    ...
    "Static error in basic_fixed_rank_extents: type must be of type integer.");
  static_assert(!std::numeric_limits<value_type>::is_signed,
                "Static error in basic_fixed_rank_extents: type must be of "
                "type unsigned integer.");

  //@returns the rank of basic_static_extents
  [[nodiscard]] static constexpr size_type size() noexcept { return _size; }
  ...
  [[nodiscard]] inline constexpr const_reference operator[](
  ...
  constexpr basic_fixed_rank_extents& operator=(
    basic_fixed_rank_extents const&
  ...
  constexpr basic_fixed_rank_extents(std::initializer_list<value_type> li)
  {
    if (li.size() > _size) {
      throw std::out_of_range(
        "boost::numeric::ublas::basic_fixed_rank_extents(std::initializer_list<"
        "value_type>): "
    ...
    if (!valid(*this)) {
      throw std::length_error(
        "Error in basic_fixed_rank_extents::basic_fixed_rank_extents() : "
  ...
  constexpr basic_fixed_rank_extents(const_iterator begin, const_iterator end)
  {
    if (std::distance(begin, end) < 0 ||
        static_cast<std::size_t>(std::distance(begin, end)) > _size) {
      throw std::out_of_range(
        "boost::numeric::ublas::basic_fixed_rank_extents(): initializer list "
        "size is greater than _size");
    }
    ...
    if (!valid(*this)) {
      throw std::length_error(
        "Error in "
        "basic_fixed_rank_extents::basic_fixed_rank_extents(const_iterator,"
        "const_iterator) : "
        "shape tuple is not a valid permutation: has zero elements.");
    }
  }

  inline constexpr void fill(value_type value) { _base.fill(value); }

  template <typename OtherExtents>
  constexpr basic_fixed_rank_extents(OtherExtents const& e)
  {
    static_assert(
      is_extents_v<OtherExtents>,
      "boost::numeric::ublas::basic_fixed_rank_extents(OtherExtents const&) : "
      "OtherExtents should be a valid tensor extents");

    if (e.size() != size()) {
      throw std::length_error(
        "Error in "
        "basic_fixed_rank_extents::basic_fixed_rank_extents(OtherExtents "
        "const&) : "
    ...
    : _base(data)
  {
    if (!valid(*this)) {
      throw std::length_error(
        "Error in basic_fixed_rank_extents::basic_fixed_rank_extents(base_type "
        "const&) : "
        "shape tuple is not a valid permutation: has zero elements.");
    }
  }
  ...
  friend void swap(basic_fixed_rank_extents& lhs,
                   basic_fixed_rank_extents&
