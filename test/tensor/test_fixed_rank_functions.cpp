...
struct fixture {
  ...
  std::tuple<fixed_rank_extents_type<2>,
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_prod_vector,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for_each_tuple(fixed_rank_extents, [](auto const&, auto& n) {
    ...
    using tensor_type =
      ublas::fixed_rank_tensor<value_type, extents_type::_size, layout_type>;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_prod_matrix,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_prod_tensor_1,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_prod_tensor_2,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  auto permute_extents_s_2 = [](auto const& pi, auto const& na) {
    auto tempn = na.base();
    assert(pi.size() == na.size());
    for (auto j = 0u; j < pi.size(); ++j) tempn[pi[j] - 1] = na[j];
    return ublas::dynamic_extents<std::decay<decltype(na)>::type::_size>(
      tempn.begin(), tempn.end());
  };
  ...
  for_each_tuple(fixed_rank_extents, [&](auto const&, auto& n) {
    ...
    for (auto f = 0ul; f < fac; ++f) {
      ...
      using tensor_type_2  = ublas::
        fixed_rank_tensor<value_type, extents_type_2::_size, layout_type>;
      auto b = tensor_type_2(nb, value_type{3});
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_inner_prod,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_fixed_rank_tensor_outer_prod,
                                 value,
                                 test_types,
                                 fixture)
{
