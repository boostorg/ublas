...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_mtv, value, test_types, fixture)
{
  ...
  for (auto const& na: extents) {
    ...
    for (auto m = 0u; m < na.size(); ++m) {
      ...
      ublas::detail::recursive::mtv(size_type(m),
                                    c.data(),
                                    nc.data(),
                                    wc.data(),
                                    a.data(),
      ...
      for (auto i = 0u; i < c.size(); ++i)
        BOOST_CHECK_EQUAL(
          c[i],
          value_type(static_cast<inner_type_t<value_type>>(na[m])) * a[i]);
    }
  }
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_mtm, value, test_types, fixture)
{
  ...
  for (auto const& na: extents) {
    ...
    ublas::detail::recursive::mtm(c.data(),
                                  nc.data(),
                                  wc.data(),
                                  a.data(),
                                  na.data(),
    ...
    for (auto i = 0u; i < c.size(); ++i)
      BOOST_CHECK_EQUAL(
        c[i], value_type(static_cast<inner_type_t<value_type>>(na[1])) * a[0]);
  }
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ttt_permutation,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for (auto const& na: extents) {
    ...
    for (auto i = 0u; i < f; ++i) {
      ...
      for (auto q = size_type(0); q <= pa; ++q) {
        ...
        for (auto j = 0u; j < r; ++j) nc_base[j] = na[pia[j] - 1];

        for (auto j = 0u; j < s; ++j) nc_base[r + j] = nb[pib_inv[j] - 1];
        ...
        for (auto j = r; j < pa; ++j)
          acc *=
            value_type(static_cast<inner_type_t<value_type>>(na[pia[j] - 1]));
