...
// using test_types =
// zip<int>::with_t<boost::numeric::ublas::layout::first_order>;


struct fixture {
  using dynamic_extents_type = boost::numeric::ublas::dynamic_extents<>;
  fixture()
    : extents{dynamic_extents_type{1, 1},         // 1
  ...
  for (auto const& n: extents) {
    ...
    for (auto m = 0u; m < n.size(); ++m) {
      ...
      for (auto i = 0u; i < c.size(); ++i)
        BOOST_CHECK_EQUAL(
          c[i], value_type(static_cast<inner_type_t<value_type>>(n[m])) * a[i]);
    }
  }
  auto n = extents[4];
  auto a = tensor_type(n, value_type{2});
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_matrix,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_tensor_1,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for (auto const& na: extents) {
    ...
    for (auto q = 0ul; q <= pa; ++q) {   // pa
      ...
      for (auto i = 0ul; i < q; ++i)
        acc *= value_type(
          static_cast<inner_type_t<value_type>>(a.extents().at(phi.at(i) - 1)));
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_tensor_2,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for (auto const& na: extents) {
    ...
    for (auto f = 0ul; f < fac; ++f) {
      ...
      for (auto q = 0ul; q <= pa; ++q) {   // pa
        ...
        auto acc = value_type(1);
        for (auto i = 0ul; i < q; ++i)
          acc *= value_type(static_cast<inner_type_t<value_type>>(
            a.extents().at(phia.at(i) - 1)));
  ...
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type{}, tensor_type({2, 1, 2}), phia, phia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2, 3}), tensor_type(), phia, phia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2, 4}), tensor_type({2, 1}), phia, phia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2}), tensor_type({2, 1, 2}), phia, phia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2}), tensor_type({2, 1, 3}), sphia, phia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2}), tensor_type({2, 2}), phia, sphia),
    std::runtime_error);
  BOOST_CHECK_THROW(
    ublas::prod(tensor_type({1, 2}), tensor_type({4, 4}), sphia, phia),
    std::runtime_error);
}
                                 ...
                                 fixture)
{
  ...
  BOOST_CHECK_THROW(
    ublas::inner_prod(tensor_type({1, 2, 3}), tensor_type({1, 2, 3, 4})),
    std::length_error);   // rank different
  BOOST_CHECK_THROW(ublas::inner_prod(tensor_type(), tensor_type()),
                    std::length_error);   // empty tensor
  BOOST_CHECK_THROW(
    ublas::inner_prod(tensor_type({1, 2, 3}), tensor_type({3, 2, 1})),
    std::length_error);   // different extent
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_norm, value, test_types, fixture)
{
  ...
  for (auto const& n: extents) {
    ...
    tensor_type var =
      (a + a) /
      2.0f;   // std::complex<float>/int not allowed as expression is captured
...
BOOST_FIXTURE_TEST_CASE(test_tensor_real_imag_conj, fixture)
{
  ...
  for (auto const& n: extents) {
    ...
    std::transform(b.begin(), b.end(), r0.begin(), [](auto const& l) {
      return std::real(l);
    });
    ...
    std::transform(b.begin(), b.end(), r00.begin(), [](auto const& l) {
      return std::conj(l);
    });
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_outer_prod,
                                 value,
                                 test_types,
                                 fixture)
{
