...
struct fixture {
    using extents_t = boost::numeric::ublas::dynamic_extents<>;
    ...
    fixture()
    {
        ...
        for(auto k = 0u; k < multi_index.size(); ++k){
            ...
            for (auto const& i : multi_index[k]){
                assert( std::equal(i.begin(), i.end(), n.begin(), std::less<>{}) ) ;
            }
        }
    }


    static inline auto shapes = std::array<extents_t,13>
        {{
            {       },
            {1,1    },
    ...
    static inline auto multi_index = std::array<std::array<multi_index_t,3>,shapes.size()>
        {{
            ...
            {{ {0,0,0  }, {1,1,0  }, {1,2,0  } }}, // 5 {2,3,1}
            {{ {0,0,0  }, {0,1,1  }, {0,1,2  } }}, // 6 {1,2,3}
            {{ {0,0,0  }, {1,0,1  }, {2,0,1  } }}, // 7 {3,1,2}
            {{ {0,0,0  }, {1,1,2  }, {2,1,3  } }}, // 8 {3,2,4}

            {{ {0,0,0,0}, {1,1,2,0}, {1,2,3,0} }}, // 9 {2,3,4,1}
            {{ {0,0,0,0}, {0,1,1,2}, {0,1,2,3} }}, //10 {1,2,3,4}
            {{ {0,0,0,0}, {1,0,1,2}, {2,0,1,3} }}, //11 {3,1,2,4}
            {{ {0,0,0,0}, {1,1,2,3}, {2,1,3,4} }}  //12 {3,2,4,5}
        }};
    ...
    static constexpr inline auto indexf = std::array<std::array<index_t,3>,shapes.size()>
        {{
            ...
            {{0, 0,  0}}, //  1 {1,1}
            {{0, 1,  1}}, //  2 {1,2}
            {{0, 1,  1}}, //  3 { {0,0    }, {1,0    }, {1,0    } }, // 3 {2,1}
            {{0, 3,  5}}, //  4 { {0,0    }, {1,1    }, {1,2    } }, // 4 {2,3}
            {{0, 3,  5}}, //  5 { {0,0,0  }, {1,1,0  }, {1,2,0  } }, // 5 {2,3,1}
            {{0, 3,  5}}, //  6 { {0,0,0  }, {0,1,1  }, {0,1,2  } }, // 6 {1,2,3}
            {{0, 4,  5}}, //  7 { {0,0,0  }, {1,0,1  }, {2,0,1  } }, // 7 {3,1,2}
            {{0,16, 23}}, //  8 { {0,0,0  }, {1,1,2  }, {2,1,3  } }, // 8 {3,2,4}, {1,3,6}
            {{0,15, 23}}, //  9 { {0,0,0,0}, {1,1,2,0}, {1,2,3,0} }, // 9 {2,3,4,1}, {1,2,6,6}
            {{0,15, 23}}, // 10 { {0,0,0,0}, {0,1,1,2}, {0,1,2,3} }, //10 {1,2,3,4}, {1,1,2,6}
            {{0,16, 23}}, // 11 { {0,0,0,0}, {1,0,1,2}, {2,0,1,3} }, //11 {3,1,2,4}, {1,3,3,6}
            {{0,88,119}}, // 12 { {0,0,0,0}, {1,1,2,3}, {2,1,3,4} }  //12 {3,2,4,5}, {1,3,6,24}
        }};
    ...
    static constexpr inline auto indexl = std::array<std::array<index_t,3>,shapes.size()>
        {{
            ...
            {{0, 0,  0}}, //  1 {1,1}
            {{0, 1,  1}}, //  2 {1,2}
            {{0, 1,  1}}, //  3 { {0,0    }, {1,0    }, {1,0    } }, // 3 {2,1    }, {1,1}
            {{0, 4,  5}}, //  4 { {0,0    }, {1,1    }, {1,2    } }, // 4 {2,3    }, {3,1}
            {{0, 4,  5}}, //  5 { {0,0,0  }, {1,1,0  }, {1,2,0  } }, // 5 {2,3,1  }, {3,1,1}
            {{0, 4,  5}}, //  6 { {0,0,0  }, {0,1,1  }, {0,1,2  } }, // 6 {1,2,3  }, {6,3,1}
            {{0, 3,  5}}, //  7 { {0,0,0  }, {1,0,1  }, {2,0,1  } }, // 7 {3,1,2  }, {2,2,1}
            {{0,14, 23}}, //  8 { {0,0,0  }, {1,1,2  }, {2,1,3  } }, // 8 {3,2,4  }, {8,4,1}
            {{0,18, 23}}, //  9 { {0,0,0,0}, {1,1,2,0}, {1,2,3,0} }, // 9 {2,3,4,1}, {12, 4,1,1}
            {{0,18, 23}}, // 10 { {0,0,0,0}, {0,1,1,2}, {0,1,2,3} }, //10 {1,2,3,4}, {24,12,4,1}
            {{0,14, 23}}, // 11 { {0,0,0,0}, {1,0,1,2}, {2,0,1,3} }, //11 {3,1,2,4}, { 8, 8,4,1}
            {{0,73,119}}, // 12 { {0,0,0,0}, {1,1,2,3}, {2,1,3,4} }  //12 {3,2,4,5}, {40,20,5,1}
        }};

    template<typename extents_type>
    constexpr inline auto prodn(extents_type const& n)
    {
        return std::accumulate(n.begin(),n.end(),1ul, std::multiplies<>{});
    }
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE( test_compute_single_index, layout_t,  layout_types, fixture )
{
    ...
    using strides_t = ub::basic_strides<value_t, layout_t>;
    ...
    mp::mp_for_each<mp::mp_iota_c<std::size(index)>>( [&]( auto I ) {
        ...
        mp::mp_for_each<mp::mp_iota_c<std::size(i)>>( [&]( auto K ) {
            ...
            auto const  j  = ub::detail::compute_single_index(ii.begin(), ii.end() ,strides_t(n).begin());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE( test_compute_single_index_static_rank, layout_t,  layout_types, fixture )
{
    ...
    mp::mp_for_each<mp::mp_iota_c<std::size(index)>>( [&]( auto I ) {
        ...
        mp::mp_for_each<mp::mp_iota_c<std::size(i)>>( [&]( auto K ) {
            auto const& ii = std::get<K>(i);
            auto const  j  = ub::detail::compute_single_index<r>(ii.begin(), ii.end() ,strides_t(n).begin());
            BOOST_CHECK(j < prodn(n));
            BOOST_CHECK_EQUAL(j,jref[K]);
        });
    });
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE( test_compute_multi_index, layout_t,  layout_types, fixture )
{
    ...
    using strides_t = basic_strides<value_t, layout_t>;

    constexpr auto is_first_order = std::is_same_v<layout_t,first_order>;
    constexpr auto const& index = is_first_order ? indexf : indexl;

    for(auto k = 0u; k < index.size(); ++k){
        auto const& n = shapes[k];
        auto const& iref = multi_index[k];
        auto const& w = strides_t(n);
        ...
        for(auto kk = 0u; kk < iref.size(); ++kk){
            ...
            //detail::compute_multi_index(jj, w.begin(), w.end(), i.begin());
            ...
            std::cout << "ii_= [ "; for(auto iii : ii) std::cout << iii << " "; std::cout << "];" << std::endl;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE( test_compute_multi_index_static_rank, layout_t,  layout_types, fixture )
{
    ...
    mp::mp_for_each<mp::mp_iota_c<std::size(index)>>( [&]( auto I ) {
        ...
        mp::mp_for_each<mp::mp_iota_c<std::size(iref)>>( [&]( auto K ) {
            ...
            auto         i = multi_index_t(w.size());
            ub::detail::compute_multi_index<r>(jj, w.begin(), w.end(), i.begin());
            BOOST_CHECK ( std::equal(i.begin(),i.end(),ii.begin()) ) ;
        });
    });
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE( test_compute_single_index_subtensor, layout_t,  layout_types, fixture )
{
    ...
    for(auto k = 0u; k < index.size(); ++k){
        ...
        for(auto kk = 0u; kk < jref.size(); ++kk){
            auto const jj = jref[kk];
            auto const j  = detail::compute_single_index(jj,w.begin(),w.end(),w.begin());
