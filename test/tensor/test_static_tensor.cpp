...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_tensor_ctor, value, test_types)
{
  ...
  auto a1 =
    ublas::static_tensor<value_type, ublas::static_extents<>, layout_type>{};
  BOOST_CHECK_EQUAL(a1.size(), 0ul);
  BOOST_CHECK(a1.empty());

  auto a2 = ublas::
    static_tensor<value_type, ublas::static_extents<1, 1>, layout_type>{};
  BOOST_CHECK_EQUAL(a2.size(), 1);
  BOOST_CHECK(!a2.empty());

  auto a3 = ublas::
    static_tensor<value_type, ublas::static_extents<2, 1>, layout_type>{};
  BOOST_CHECK_EQUAL(a3.size(), 2);
  BOOST_CHECK(!a3.empty());

  auto a4 = ublas::
    static_tensor<value_type, ublas::static_extents<1, 2>, layout_type>{};
  ...
  auto a6 = ublas::
    static_tensor<value_type, ublas::static_extents<4, 3, 2>, layout_type>{};
  BOOST_CHECK_EQUAL(a6.size(), 4 * 3 * 2);
  BOOST_CHECK(!a6.empty());

  auto a7 = ublas::
    static_tensor<value_type, ublas::static_extents<4, 1, 2>, layout_type>{};
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_ctor,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for_each_tuple(extents, [](auto const&, auto& e) {
    ...
    for (auto i = 0ul; i < t.size(); ++i) BOOST_TEST(t[i] == r[i]);
  });
}


BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_ctor_layout,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  using other_layout_type = std::conditional_t<
    std::is_same<ublas::layout::first_order, layout_type>::value,
  ...
  for_each_tuple(extents, [](auto const&, auto& e) {
    using extents_type = std::decay_t<decltype(e)>;
    using tensor_type =
      ublas::static_tensor<value_type, extents_type, layout_type>;
    ...
    for (auto i = 0ul; i < t.size(); ++i) BOOST_TEST(q[i] == r[i]);
  });
}


BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_move_ctor,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents_init,
                                 value,
                                 test_types,
                                 fixture)
{
  ...
  for_each_tuple(extents, [&](auto const&, auto const& e) {
    ...
    auto r = value_type(
      static_cast<inner_type_t<value_type>>(distribution(generator)));
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents_array,
                                 value,
                                 test_types,
                                 fixture)
{
                                 ...
                                 fixture)
{
  ...
  auto check2 = [](const auto& t) {
    ...
    auto r =
      std::is_same<layout_type, ublas::layout::first_order>::value ? 1 : 0;
    auto q =
      std::is_same<layout_type, ublas::layout::last_order>::value ? 1 : 0;
    auto v = value_type{};
  ...
  auto check3 = [](const auto& t) {
    ...
    using op_type = std::conditional_t<
      std::is_same_v<layout_type, ublas::layout::first_order>,
      std::minus<>,
      std::plus<>>;
  ...
  auto check = [check1, check2, check3, check4](auto const&, auto const& e) {
    ...
    if constexpr (extents_type::_size == 1)
      check1(t);
    else if constexpr (extents_type::_size == 2)
      check2(t);
    else if constexpr (extents_type::_size == 3)
      check3(t);
    else if constexpr (extents_type::_size == 4)
      check4(t);
  };
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_standard_iterator,
                                 value,
                                 test_types,
                                 fixture)
{
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_throw, value, test_types, fixture)
{
  ...
  using layout_type = typename value::second_type;
  using tensor_type =
    ublas::static_tensor<value_type, ublas::static_extents<5, 5>, layout_type>;
