...
//FIXME:  BOOST_CHECK( ublas::is_valid (extents<3>{}) );
  ...
  BOOST_CHECK( ublas::is_valid(extents<1,2>{}) );
  BOOST_CHECK( ublas::is_valid(extents<1,3>{}) );
  ...
  for_each_in_tuple(scalars  ,[](auto const& /*unused*/, auto& e){ BOOST_CHECK(  ublas::is_valid(decltype(e){}) ); });
  for_each_in_tuple(vectors  ,[](auto const& /*unused*/, auto& e){ BOOST_CHECK(  ublas::is_valid(decltype(e){}) ); });
  for_each_in_tuple(matrices ,[](auto const& /*unused*/, auto& e){ BOOST_CHECK(  ublas::is_valid(decltype(e){}) ); });
  for_each_in_tuple(tensors  ,[](auto const& /*unused*/, auto& e){ BOOST_CHECK(  ublas::is_valid(decltype(e){}) ); });
}
